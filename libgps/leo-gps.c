                    ...
                    if (cmd == CMD_QUIT) {
                        ...
                        if (!started) {
                            ...
                            if ( pthread_create( &state->tmr_thread, NULL, gps_timer_thread, state ) != 0 ) {
                                ALOGE("could not create gps_timer_thread: %s", strerror(errno));
                                started = 0;
...
static void gps_state_init( GpsState*  state ) {
    ...
    if ( sem_init(&state->fix_sem, 0, 1) != 0 ) {
        ALOGE("gps semaphore initialization failed: %s", strerror(errno));
        goto Fail;
    }
#endif
    ...
    if ( pthread_create( &state->pos_thread, NULL, gps_get_position_thread, NULL ) != 0 ) {
        ALOGE("could not create gps_get_position_thread: %s", strerror(errno));
        goto Fail;
    }
